...
export default function GameDetailPage() {
  ...
  const fetchGame = useCallback(async () => {
    ...
    const ownershipPromise = user && token
      ? fetch(`/api/library/ownership?gameId=${encodeURIComponent(g.id)}`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((res) => res.ok ? res.json() : { owned: false }).catch(() => ({ owned: false }))
      : Promise.resolve({ owned: false });

    const authStatusPromise = user && userId !== 'guest'
      ? Promise.all([
    ...
    const [reviewsRes, relatedRes, ownership, authStatus] = await Promise.all([
    ...
    const [wishCheck, cartCheck, wishAll, cartAll] = authStatus;

    setIsOwned(Boolean((ownership as { owned?: boolean }).owned));
    ...
    setOwnedIds(new Set(Boolean((ownership as { owned?: boolean }).owned) ? [g.id] : []));
