...
import { Star, Download, ShoppingCart, Heart, Check, ArrowLeft, Calendar, Monitor, User, Building2, Tag, Play, Loader as Loader2, MessageSquare, Send, ChevronLeft, ChevronRight, Cpu, HardDrive, MemoryStick, Film, Images } from 'lucide-react';
...
export default function GameDetailPage() {
  ...
  const fetchGame = useCallback(async () => {
    ...
    if (!gameRes.data && !fallbackGame) {
      setLoading(false);
      return;
    }

    const merged = (gameRes.data as Game | null) ?? fallbackGame;
    const g = (merged
      ? normalizeDbGameRow(merged as unknown as Record<string, unknown>)
      : null) as Game;
    setGame(g);

    if (isSeedGameId(g.id)) {
      const related = TOP_GAME_SEEDS
        .filter(item => item.id !== g.id && item.genre.some(genre => g.genre.includes(genre)))
        .slice(0, 4);
    ...
    const relatedPromise =
      (g.genre ?? []).length > 0
        ? supabase.from('games').select('*').neq('id', g.id).overlaps('genre', g.genre).limit(4)
        : supabase.from('games').select('*').neq('id', g.id).order('rating', { ascending: false }).limit(4);

    const [libCheck, wishCheck, cartCheck, reviewsRes, relatedRes, libAll, wishAll, cartAll] = await Promise.all([
      supabase.from('library').select('id').eq('user_id', userId).eq('game_id', g.id).maybeSingle(),
      supabase.from('wishlist').select('id').eq('user_id', userId).eq('game_id', g.id).maybeSingle(),
      supabase.from('cart').select('id').eq('user_id', userId).eq('game_id', g.id).maybeSingle(),
      supabase.from('reviews').select('*, profiles(username)').eq('game_id', g.id).order('created_at', { ascending: false }),
      relatedPromise,
    ...
    setIsOwned(!!libCheck.data);
    setIsWishlisted(!!wishCheck.data);
    setIsInCart(!!cartCheck.data);
    setReviews((reviewsRes.data ?? []) as Review[]);
    setRelatedGames(
      (relatedRes.data ?? []).map((row) =>
        normalizeDbGameRow(row as unknown as Record<string, unknown>)
      ) as Game[]
    );
    setOwnedIds(new Set((libAll.data ?? []).map((l: any) => l.game_id)));
  ...
  async function addToLibrary() {
    ...
    const { error } = await supabase.from('library').upsert({ user_id: userId, game_id: game.id }, { onConflict: 'user_id,game_id' });
    if (error) {
      toast.error(error.message);
      setActionLoading(null);
      return;
    }
    await supabase.from('cart').delete().eq('user_id', userId).eq('game_id', game.id);
    setIsOwned(true);
    setIsInCart(false);
    toast.success(`${game.title} added to your library`);
    setActionLoading(null);
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="min-w-0 space-y-8">
            ...
            <section className="overflow-hidden rounded-lg border border-[#3a3a3a] bg-[#1f1f1f] shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
              <div className="flex flex-col gap-3 border-b border-[#2a2a2a] px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-4 mt-28 rounded-[1.75rem] border border-white/10 bg-slate-950/90 p-6 ring-1 ring-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.25)] backdrop-blur-xl">
              ...
              <div className="space-y-2">
                {isOwned ? (
                  ...
                  <>
                    <button onClick={addToCart} disabled={isInCart || actionLoading === 'cart'} className={cn('w-full flex items-center justify-center gap-2 py-3 font-bold rounded-xl transition-colors', isInCart ? 'bg-green-500/20 text-green-400 border border-green-500/30 cursor-default' : 'bg-sky-500 hover:bg-sky-400 text-white shadow-lg shadow-sky-500/20')}>
                    ...
                    <Link href="/cart" className="w-full flex items-center justify-center gap-2 py-2.5 bg-white/10 hover:bg-white/15 text-white font-medium rounded-xl transition-colors text-sm">View Cart</Link>
                  </>
                )}
