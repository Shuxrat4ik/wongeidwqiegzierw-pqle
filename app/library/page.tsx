...
export default function LibraryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const res = await supabase.from('library').select('id, game_id, acquired_at, games(*)').eq('user_id', userId).order('acquired_at', { ascending: false });
      const mapped: LibraryEntry[] = (res.data ?? [])
        .filter((l: { games?: unknown }) => l.games)
        .map((l: { id: string; acquired_at: string; games: unknown }) => ({
          id: l.id,
          game: normalizeDbGameRow(l.games as Record<string, unknown>) as Game,
